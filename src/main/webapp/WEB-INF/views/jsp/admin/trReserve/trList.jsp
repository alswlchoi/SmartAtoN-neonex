...
$(document).on("click",".godetail",function(){
	...
	var form = $("<form></form>");
	form.attr("action","/admin/trReserve/tr-detail");
	form.attr("method","post");
	form.appendTo("body");
	form.append('<input type="hidden" value='+tcSeq+' name="tcSeq" />');
	form.append('<input type="hidden" value='+tcDay+' name="tcDay" />');
	form.append('<input type="hidden" value='+tcStep+' name="tcStep" />');
	...
	if(rows.length==0){
		...
		for(var list in rows){
			...
			if(undefinedChk(rows[list].compCode,"")=="THINT"){
				html += '<br /><span class="color_red">'+undefinedChk(rows[list].tcRequestNumber,"")+'</span>';
			}
			html += '</td>';
			var tcRegDt = "";
...
function drawRfidSelect(rows, currentrId){
	...
	for(var list in rows){
		html += '<option value="'+undefinedChk(rows[list].rId,"")+'">'+undefinedChk(rows[list].rId,"")+'</td>';
	}
	html += '</select>';
	$("#rfidlist").html(html)
}
...
//무전기 정보 selectbox 생성
function drawWiressSelect(rows, currentwId){
	var html='';
	html += '<select name="wId" id="wId">';
	html += '<option value="">무전기선택</option>';
	for(var list in rows){
		html += '<option value="'+undefinedChk(rows[list].wId,"")+'">'+undefinedChk(rows[list].wId,"")+'</td>';
	}
	html += '</select>';
	$("#wIdlist").html(html)
}
                                                ...
                                                <table class="tbl_list" summary="예약승인대기목록테이블 입니다. 항목으로는 NO, 예약번호, 접수일자, 시험일자, 회사명, 항목, 유형등이 있습니다">
                                                    ...
                                                    <thead>
                                                        <tr>
                                                            ...
                                                            <th scope="col"><button id="order5">유형</button>
