...
$(document).ready(function(){
	...
	$(document).on("click","#btnSearch",function(){
		...
		location.href=window.location.pathname+"?year="+${today_info.search_year}+"&month="+(${today_info.search_month}-1)+"&reqTrackType="+reqTrackType+"&reqCompType="+reqCompType+"&reqTrackCode="+reqTrackCode+"&tcApproval=3&tcStep=0&viewKind=detail";
	});		
...
function drawingDetailTable(trReserve, driver, company){
	...
	html += '<thead>';		
	html += '<tr>';		
	html += '<th>예약번호</th><th>접수일자</th><th>시험일자</th><th>회사명</th><th>항목</th><th>유형</th><th>진행상태</th>';		
	html += '</tr>';
	html += '</thead>';		
	html += '<tbody>';
	...
	for(var list in trackInfo) {
		if(list == 0){
			trTrackNickName = trackInfo[list].trTrackNickName;
			trTrackType = trackInfo[list].trTrackType;
		}else{
			if(trTrackNickName.indexOf(undefinedChk(trackInfo[list].trTrackNickName,""))<0){
				trTrackNickName += "<br />"+undefinedChk(trackInfo[list].trTrackNickName,"");
			}
		}
	}
	html += trTrackNickName;
	html += '</td>';
	html += '<td>';
	...
	html += '</td>';
	var tcApproval = undefinedChk(trReserve.tcApproval,"");
	var tcStep = undefinedChk(trReserve.tcStep,"");
	html += '<td id="appTxt">';
	if(tcApproval=="0"){
		html += '승인대기';
	}else if(tcApproval=="3") {
		if(tcStep=="00000"){
			html += '시험전';
	...
	if(undefinedChk(trReserve.compCode,"")!="THINT"){
		...
		html += '		</tr>';
		html += '		<tr>';
		html += '			<th scope="row">이메일주소</th><td colspan="3">'+undefinedChk(company.memEmail,"")+'</td>';
		html += '		</tr>';
		html += '		<tr>';
		html += '			<th scope="row">회계담당자</th><td>'+undefinedChk(company.compAcctName,"")+'</td>';
		html += '			<th scope="row">부서</th><td>'+undefinedChk(company.compAcctDept,"")+'</td>';
		html += '		</tr>';
		html += '		<tr>';
		html += '			<th scope="row">이메일주소</th><td>'+undefinedChk(company.compAcctEmail,"")+'</td>';
		html += '			<th scope="row">전화번호</th><td>';
		var compAcctPhone = undefinedChk(company.compAcctPhone,"")
		if(compAcctPhone!=""){
			...
			}else if(compAcctPhone.substring(0,3)=="010"){
				compAcctPhone = compAcctPhone.substring(0,3)+'-'+compAcctPhone.substring(3,7)+'-'+compAcctPhone.substring(7,11);
			}else{
				if(compAcctPhone.length==10){
					compAcctPhone = compAcctPhone.substring(0,3)+'-'+compAcctPhone.substring(3,6)+'-'+compAcctPhone.substring(6,10);
				}else if(compAcctPhone.length==11){
					compAcctPhone = compAcctPhone.substring(0,3)+'-'+compAcctPhone.substring(3,7)+'-'+compAcctPhone.substring(7,11);
				}else if(compAcctPhone.length==12){
...
function drawingTable(rows, paging){
	...
	if(rows.length==0){
		...
		for(var list in rows){
			...
			html += '</td>';
			var tcApproval = undefinedChk(rows[list].tcApproval,"");
                                    ...
                                    <ul>
                                        ...
                                        <li class="tabCot">
                                            ...
                                            <div class="calendar_wrap2">
                                                <div class="btn_cal">
                                                    <a href="/admin/trReserve/trcalendar?viewKind=detail&amp;year=${today_info.before_year }&amp;month=${today_info.before_month }&amp;reqTrackType=${reqTrackType }&amp;reqCompType=${reqCompType }&amp;reqTrackCode=${reqTrackCode }&amp;tcApproval=3&amp;tcStep=0"
                                                    target="_self" class="prev">
                                                    </a>
                                                    <span>${today_info.search_year}년 
                                                    <c:if test="${today_info.search_month<10}">0</c:if>${today_info.search_month}월
                                                    </span>
                                                    <a href="/admin/trReserve/trcalendar?viewKind=detail&amp;year=${today_info.after_year}&amp;month=${today_info.after_month}&amp;reqTrackType=${reqTrackType }&amp;reqCompType=${reqCompType }&amp;reqTrackCode=${reqTrackCode }&amp;tcApproval=3&amp;tcStep=0" 
                                                    target="_self" class="next"></a>
                                                </div>
                                                <!-- search_wrap -->
                                                ...
                                                <section class="tbl_wrap_list_cal m-t-10">
                                                    <table class="tbl_list" summary="테이블 입니다. 항목으로는 등이 있습니다">
														...
														<c:forEach var="dateList" items="${dateList}" varStatus="date_status">
															...
															<tr>	
																<td valign="top" class="<c:if test="${dateList.value=='today'}">today_style </c:if>sun_day">
																${dateList.date}
																<span class="capa">${dateList.strCnt}</span>
																${dateList.schedule_detail}
																<c:if test="${dateList.schedule > 4}"><a href="#" id="dt${aid }" class="more" data-layer-top="more_cal">+ more</a></c:if></td>
