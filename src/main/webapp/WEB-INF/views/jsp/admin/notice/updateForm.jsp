...
function fileDelete(fSeq){
	var con = confirm("삭제된 파일은 복구할 수 없습니다.\n파일을 삭제하시겠습니까?");
...
$(document).ready(function(){	
	$(document).on("click",'.listBtn' ,function(){
		location.href="/admin/notice?nType=${nType}&schTop=${schTop}&pageNo=${pageNo}";
	});
	...
	$("#regBtn").click(function(){
		...
		*/
		$.ajax({
			url : "/admin/notice/fileupload/update-notice",
			type: "post",
			...
			success : function(resdata){
				if(resdata.code == 400){
					...
					location.href="/admin/notice?nType=${nType}&schTop=${schTop}";
				}
			},
			error : function(e){
				console.log(e);
	...
	$(document).on("click",'#delBtn' ,function(){
		confirm("삭제된 정보는 복구되지 않습니다. 정말 삭제하시겠습니까?");
		nSeq = $("#nSeq").val();
	...
	$(document).on("change",'input[id^=nFileS]' ,function(){
		...
		if($.inArray(ext, ['txt','png','jpg','jpeg','GIF','TXT','PNG','JPG','JPEG','bmp','tiff','tif','pdf']) == -1) {
        ...
        <div id="container">
            ...
            <div class="content">
				...
				<div id="data_area" class="form_content form_group_wrap">
					<form name="form" id="form" method="post" action="/admin/notice/fileupload/update-notice" enctype="multipart/form-data">
					<input type="hidden" name="nSeq" id="nSeq" value="${notice.NSeq }" />
					<input type="hidden" name="nType" id="nType" value="${nType}" />
					<input type="hidden" name="schTop" id="schTop" value="${schTop}" />
	                        ...
	                        <tr>
	                            ...
	                            <td>
	                                <div class="form_group w_full">
	                                    <input type="text" id="nTitle" class="form_control" placeholder="제목 입력" name="nTitle" value="${notice.NTitle }" />
	                                </div>
	                                ...
	                                <div class="form_group">
	                                    <div class="radio_inline">
	                                        <label class="radio_default">
	                                            <input type="radio" name="nTop" id="nTopY" value="Y"<c:if test="${notice.NTop=='Y' }"> checked="checked"</c:if> />
	                                            <span class="radio_icon"></span>노출</label>
	                                        <label class="radio_default">
	                                            <input type="radio" name="nTop" id="nTopY" value="N"<c:if test="${notice.NTop=='N' }"> checked="checked"</c:if> />
	                            ...
	                            <td colspan="3">
	                                <div class="form_group w_full">
	                                    <textarea name="" id="nContent" cols="" rows="5" class="form_control h500"
	                                        placeholder="공지사항 내용 입력">${notice.NContent }</textarea>
	                                </div>
	                            ...
	                            <td colspan="3">
	                                ...
	                                <section>
	                                    ...
	                                    <c:forEach var="result" items="${file}" varStatus="status">
											<li id="li${result.FSeq }">
												<a href="/admin/notice/fileupload/download/${result.FSeq }">${result.FName }</a>
												<button onclick="javascript:fileDelete(${result.FSeq });return false;"></button>
											</li>
										</c:forEach>
	                                    </ul>
	                                </section>
	                ...
	                <section class="btn_wrap m-t-50">
	                    ...
	                    <section>
	                        <button type="button" id="delBtn" class="btn btn_gray m-r-6">삭제</button>
