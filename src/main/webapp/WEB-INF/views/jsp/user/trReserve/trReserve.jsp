...
var shopReserveCntArr = new Array();
$(document).ready(function () {	
	...
	$(document).on("click",'#confirmBtn' ,function(){	//데이터 유효성 체크
		...
		for(var i=0; i<$("input[name=cNumber]").length; i++){
			...
			var cNumber_trim = cNumberArr[i].replace(/ /g,"");
			...
			if(cNumber_trim==""){
				...
				$("#confirmBtnAfter").prop("id", "confirmBtn").removeClass("btn_gray");
				return false;
			}else if(cNumber_trim.length>12){
				$("#errcNumber").text("차량번호를 형식에 맞게 입력해 주세요.")
				.addClass("redfont")
				.addClass("info_ment");
				$("#confirmBtnAfter").prop("id", "confirmBtn").removeClass("btn_gray");
				return false;
		...
		$("button[id^=bcnt]").each(function( index ) {
			if($(this).hasClass("plan")){
				...
				var cnt = $("#cnt"+shopReserve[index]).text();
				if(undefinedChk(cnt,"")==""){
					shopReserveCntArr[index] = 0;	
				}else{
					shopReserveCntArr[index] = cnt;
				}
			}else{
				shopReserveCntArr[index] = 0;
			}
		...
		var data ={
				...
				shopReserveCntArr : shopReserveCntArr				
		};
    ...
    shopPrice();
  	...
  	$.ajax({
  		...
  		success : function(resdata){
  			...
  			viewCharge(resdata.trackList, resdata.entranceFee);
  		},
  		error : function(e){
  			console.log(e);
  		}
	...
	function drawSTrackSelect(trackList){
		strackHtml +='<li>';
		strackHtml +='<div class="trdiv form_group w200">';
	...
	function drawMTrackSelect(trackList){
		mtrackHtml +='<li>';
		mtrackHtml +='<div class="trdiv form_group w200">';
	...
	function viewCharge(trackList, entranceFee){
		var html = '';
		for(var list in trackList){
			if(list>0){
				html += '<tr>';
				html += '    <td>'+(parseInt(list))+'</td>';
   ...
   function setDisable(list){
     if (list.weekday.length > 0) {
         var data = list.weekday[0];

         var begin = data.wdStDt;
         var end = data.wdEdDt;
         wdDay = data.wdDay;
     }
     if (list.dayOff.length > 0) {
       for (var i in list.dayOff) {
         ...
         var begin = new Date(changeDate(data.doStDay));
         var end = new Date(changeDate(data.doEdDay));
         for (;begin <= end; begin.setDate(begin.getDate()+1)) {
        	 if(wdDay.indexOf(begin.getDay())>=0){
     ...
     $('#tcDay').daterangepicker({
       ...
       isInvalidDate: function(date) {
    	   ...
    	   var resultVal = true;
    	   if(wdDay.indexOf(dateDay)>=0){
	   		   if(dayOff.includes(date.format("YYYY-MM-DD"))){
				   resultVal = true;
			   }else{
		    	  resultVal = false;
			   }
           ...
           //console.log("date : " + date.format("YYYY-MM-DD"), "bool : " + resultVal);
           if(resultVal){
               return true;
           }
           else{
	...
	$(document).on("click",'#searchBtn' ,function(){		
		...
		if (tcDays==""){
			...
			if(false) {		
				...
				$.ajax({
					url : "/user/trReserve/shop-can-list",
					type : "get",
					data : {
						"tcDay":tcDays
					},
	...
	function drawingShopTable(shop) {
		if(shop.error=="cannot"){
			alert("선택하신 기간에 이미 예약내역이 존재합니다.");
			$("#tcDay").val("");
		}else if(shop.error=="notlogin"){
			alert("재로그인 후 이용해 주세요.");
			...
			if(undefinedChk(shop, "")!=""){
				for (const [key, value] of Object.entries(shop)) {
					if(key.indexOf("cnt")>-1){
						$("#"+key).text(value);
						if(parseInt(value)>0){
							$("#b"+key).removeClass("plan").addClass("can_reserve").attr("disabled", false).text("예약가능").css({ background:"#fff" , color:"#000" });
						}else{
							$("#b"+key).removeClass("plan").removeClass("can_reserve").attr("disabled", true).text("예약불가").css({ background:"#fff" , color:"#999" });						
						}
					}else{
						if($("#tcDay").val()!=""){
...
<script type="text/javascript" src="/inc/js/trReserve/reserve.js"></script>
        ...
        <div id="container">
            ...
            <div class="content">
                        ...
                        <tbody>
                            ...
                            <c:forEach var="result" items="${shopList}" varStatus="status">
                                <td>
                                    ...
                                    <p><button id="bcnt${result.wsCode }" type="button" class="btn-line-s btn_gray" disabled>예약가능</button></p>
                                </td>
                            </c:forEach>
                            </tr>
                        </tbody>
                ...
                <!-- 시험유형-공동 -->
                <!-- accordion -->
                <div class="wrap_accordion2 m-t-20">
                	<span class="info_ment_orange" style="margin-bottom: 20px; margin-top:0px;">입차 차량별 GNR GATE 입차 입장료(entrance fee)가 부과되니 예약에 참고바랍니다.</span>
                                ...
                                <tr>
                                    ...
                                    <td class="p-t-0">
                                        <ul class="list_test01" id="strack">
                                        	<li>
												<div class="trdiv form_group w200">
                ...
                <section class="tac m-t-50">
                    ...
                    <button type="button" id="cancelBtn" class="btn btn_gray m-r-11" onclick="javascript:history.back()">취소</button>
    ...
    <div class="ly_group">
        <article id="printarea" class="layer_xxl charge printarea">
            ...
            <div class="ly_con">
				...
				<!-- 기본요금 -->
				<h3 class="stitle m-t-57">기본사용 요금<span class="fsd fr m-t-4">(단위:원, VAT 별도)</span></h3>
                <!-- table list -->
                <section class="tbl_wrap_list m-t-10">
                    <table class="tbl_list" summary="테이블 입니다. 항목으로는 등이 있습니다">
                        <caption>테이블</caption>
                ...
                <section class="tbl_wrap_list m-t-10">
                    ...
                    <span class="info_ment m-l-0 m-t-5 disb">복수 시험로 사용 시 높은 단가의 시험로 요금이 적용되며 사용시간 초과시 1시간 단위로 과금되니 이용에
                        참고바랍니다.</span>
                    <span class="info_ment m-l-0 m-t-10 disb">입차 차량별 GNR GATE 입차 입장료(entrance fee)가 부과되니 예약에 참고바랍니다..</span>
                </section>
                <!-- //table list -->
